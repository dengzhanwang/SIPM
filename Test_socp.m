...
for test = 1
    for k = 1:2
        ...
        opts.methods = 'mom';
        opts.lr = @(i) min(max(4/i^(1/1.8),0.0001),1)*0.005;
        tt = tic;
        ...
        figure(1)
        semilogy(out2.error(1:batchnum:end),'b-','LineWidth',3) % Changed line style to '-' and LineWidth increased

        opts.gamma = 0.1;
        opts.methods = 'igt';
        opts.lr = @(i) min(max(4/i^(1/1.9),0.0001),1)*0.005;
        tt = tic;
        ...
        figure(1)
        semilogy(out3.error(1:batchnum:end),'k--','LineWidth',3) % Changed line style to '--' and LineWidth increased
        %
        opts.gamma = 0.1;
        opts.radius = 4e10;
        ...
        if opts.batchsize == 500 && n0 == 10000
           figure(1)
        ...
        fvaltable(test,k,7) = out_al.ferror;
        % errortable(test,k,5) = out_all.error(end);
        timetable(test,k,7) = out_al.time;
